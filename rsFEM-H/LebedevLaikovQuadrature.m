function [LLQ_points LLQ_weights] = LebedevLaikovQuadrature(degree)

%  Robert Parrish (2022). getLebedevSphere (https://www.mathworks.com/matlabcentral/fileexchange/27097-getlebedevsphere), MATLAB Central File Exchange. Retrieved July 30, 2022. 
...
% degree: {1, 6, 14, 26, 38, 50, 74, 86, 110, 146, 170, 194, 230, 266, 302, 
...
if degree ~=1
leb_tmp = zeros(degree,4); %columns: x, y, z, w
start = 1;
a = 0.0;
b = 0.0;
c = 0.0;
...
%leb_tmp.n = degree;
LLQ_points(1:degree,1) = leb_tmp(:,1);
LLQ_points(1:degree,2) = leb_tmp(:,2);
LLQ_points(1:degree,3) = leb_tmp(:,3);
LLQ_weights = leb_tmp(:,4)./(4*pi);
else
    LLQ_weights = 1;
    LLQ_points = [sqrt(1/3) sqrt(1/3) sqrt(1/3)];
end
end
...
switch (type)
    ...
    case 1
        ...
        leb(start,2) = 0.0;
        leb(start,3) = 0.0;
        leb(start,4) = 4.0*pi*v;

        leb(start+1,1) = -a;
        leb(start+1,2) = 0.0;
        leb(start+1,3) = 0.0;
        leb(start+1,4) = 4.0*pi*v;

        leb(start+2,1) = 0.0;
        leb(start+2,2) = a;
        leb(start+2,3) = 0.0;
        leb(start+2,4) = 4.0*pi*v;

        leb(start+3,1) = 0.0;
        leb(start+3,2) = -a;
        leb(start+3,3) = 0.0;
        leb(start+3,4) = 4.0*pi*v;

        leb(start+4,1) = 0.0;
        leb(start+4,2) = 0.0;
        leb(start+4,3) = a;
        leb(start+4,4) = 4.0*pi*v;

        leb(start+5,1) = 0.0;
        leb(start+5,2) = 0.0;
        leb(start+5,3) = -a;
        leb(start+5,4) = 4.0*pi*v;
        start = start+6;
    ...
    case 2
        ...
        leb(start+6,2) = 0.0;
        leb(start+6,3) = a;
        leb(start+6,4) = 4.0*pi*v;

        leb(start+7,1) = -a;
        leb(start+7,2) = 0.0;
        leb(start+7,3) = -a;
        leb(start+7,4) = 4.0*pi*v;

        leb(start+8,1) = a;
        leb(start+8,2) = a;
        leb(start+8,3) = 0.0;
        leb(start+8,4) = 4.0*pi*v;

        leb(start+9,1) = -a;
        leb(start+9,2) = a;
        leb(start+9,3) = 0.0;
        leb(start+9,4) = 4.0*pi*v;

        leb(start+10,1) = a;
        leb(start+10,2) = -a;
        leb(start+10,3) = 0.0;
        leb(start+10,4) = 4.0*pi*v;

        leb(start+11,1) = -a;
        leb(start+11,2) = -a;
        leb(start+11,3) = 0.0;
        leb(start+11,4) = 4.0*pi*v;
        start = start+12;
    ...
    case 4
        ...
        leb(start+12,2) = b;
        leb(start+12,3) = -a;
        leb(start+12,4) = 4.0*pi*v;

        leb(start+13,1) = a;
        leb(start+13,2) = -b;
        leb(start+13,3) = -a;
        leb(start+13,4) = 4.0*pi*v;

        leb(start+14,1) = -a;
        leb(start+14,2) = -b;
        leb(start+14,3) = -a;
        leb(start+14,4) = 4.0*pi*v;

        leb(start+15,1) = a;
        leb(start+15,2) = b;
        leb(start+15,3) = a;
        leb(start+15,4) = 4.0*pi*v;

        leb(start+16,1) = b;
        leb(start+16,2) = a;
        leb(start+16,3) = a;
        leb(start+16,4) = 4.0*pi*v;

        leb(start+17,1) = -b;
        leb(start+17,2) = a;
        leb(start+17,3) = a;
        leb(start+17,4) = 4.0*pi*v;

        leb(start+18,1) = b;
        leb(start+18,2) = -a;
        leb(start+18,3) = a;
        leb(start+18,4) = 4.0*pi*v;

        leb(start+19,1) = b;
        leb(start+19,2) = a;
        leb(start+19,3) = -a;
        leb(start+19,4) = 4.0*pi*v;

        leb(start+20,1) = -b;
        leb(start+20,2) = -a;
        leb(start+20,3) = a;
        leb(start+20,4) = 4.0*pi*v;

        leb(start+21,1) = -b;
        leb(start+21,2) = a;
        leb(start+21,3) = -a;
        leb(start+21,4) = 4.0*pi*v;

        leb(start+22,1) = b;
        leb(start+22,2) = -a;
        leb(start+22,3) = -a;
        leb(start+22,4) = 4.0*pi*v;

        leb(start+23,1) = -b;
        leb(start+23,2) = -a;
        leb(start+23,3) = -a;
        leb(start+23,4) = 4.0*pi*v;
        start = start + 24;
    ...
    case 6
        ...
        leb(start+24,2) = b;
        leb(start+24,3) = a;
        leb(start+24,4) = 4.0*pi*v;

        leb(start+25,1) = -c;
        leb(start+25,2) = b;
        leb(start+25,3) = a;
        leb(start+25,4) = 4.0*pi*v;

        leb(start+26,1) = c;
        leb(start+26,2) = -b;
        leb(start+26,3) = a;
        leb(start+26,4) = 4.0*pi*v;

        leb(start+27,1) = c;
        leb(start+27,2) = b;
        leb(start+27,3) = -a;
        leb(start+27,4) = 4.0*pi*v;

        leb(start+28,1) = -c;
        leb(start+28,2) = -b;
        leb(start+28,3) = a;
        leb(start+28,4) = 4.0*pi*v;

        leb(start+29,1) = c;
        leb(start+29,2) = -b;
        leb(start+29,3) = -a;
        leb(start+29,4) = 4.0*pi*v;

        leb(start+30,1) = -c;
        leb(start+30,2) = b;
        leb(start+30,3) = -a;
        leb(start+30,4) = 4.0*pi*v;

        leb(start+31,1) = -c;
        leb(start+31,2) = -b;
        leb(start+31,3) = -a;
        leb(start+31,4) = 4.0*pi*v;

        leb(start+32,1) = a;
        leb(start+32,2) = c;
        leb(start+32,3) = b;
        leb(start+32,4) = 4.0*pi*v;

        leb(start+33,1) = -a;
        leb(start+33,2) = c;
        leb(start+33,3) = b;
        leb(start+33,4) = 4.0*pi*v;

        leb(start+34,1) = a;
        leb(start+34,2) = -c;
        leb(start+34,3) = b;
        leb(start+34,4) = 4.0*pi*v;

        leb(start+35,1) = a;
        leb(start+35,2) = c;
        leb(start+35,3) = -b;
        leb(start+35,4) = 4.0*pi*v;

        leb(start+36,1) = -a;
        leb(start+36,2) = -c;
        leb(start+36,3) = b;
        leb(start+36,4) = 4.0*pi*v;

        leb(start+37,1) = a;
        leb(start+37,2) = -c;
        leb(start+37,3) = -b;
        leb(start+37,4) = 4.0*pi*v;

        leb(start+38,1) = -a;
        leb(start+38,2) = c;
        leb(start+38,3) = -b;
        leb(start+38,4) = 4.0*pi*v;

        leb(start+39,1) = -a;
        leb(start+39,2) = -c;
        leb(start+39,3) = -b;
        leb(start+39,4) = 4.0*pi*v;

        leb(start+40,1) = b;
        leb(start+40,2) = c;
        leb(start+40,3) = a;
        leb(start+40,4) = 4.0*pi*v;

        leb(start+41,1) = -b;
        leb(start+41,2) = c;
        leb(start+41,3) = a;
        leb(start+41,4) = 4.0*pi*v;

        leb(start+42,1) = b;
        leb(start+42,2) = -c;
        leb(start+42,3) = a;
        leb(start+42,4) = 4.0*pi*v;

        leb(start+43,1) = b;
        leb(start+43,2) = c;
        leb(start+43,3) = -a;
        leb(start+43,4) = 4.0*pi*v;

        leb(start+44,1) = -b;
        leb(start+44,2) = -c;
        leb(start+44,3) = a;
        leb(start+44,4) = 4.0*pi*v;

        leb(start+45,1) = b;
        leb(start+45,2) = -c;
        leb(start+45,3) = -a;
        leb(start+45,4) = 4.0*pi*v;

        leb(start+46,1) = -b;
        leb(start+46,2) = c;
        leb(start+46,3) = -a;
        leb(start+46,4) = 4.0*pi*v;

        leb(start+47,1) = -b;
        leb(start+47,2) = -c;
        leb(start+47,3) = -a;
        leb(start+47,4) = 4.0*pi*v;
        start = start + 48;
