...
function [density r_on_ip thetas phis] = densityWithoutDFT(E,N,GQ,value_PHI,value_diff_PHI,coord,Clm,connectivity,NumElmNods,LLQ,L_cutoff)
N2 = 183; % angular resolution for density
phis = [pi/2*ones(1,(N2+1)/2) 3*pi/2*ones(1,(N2-1)/2)];
theta_first_half = [linspace(0,pi,(N2+1)/2)];
theta_Second_half = [linspace(-pi,0,(N2+1)/2)];
thetas = [theta_first_half theta_Second_half(2:end)];
density = zeros(E*GQ,length(thetas));
fractional_occupancy = [0.5]; 
for e = 1:E
    for j=1:GQ
